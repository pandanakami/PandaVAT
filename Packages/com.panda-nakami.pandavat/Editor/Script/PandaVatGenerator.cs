...
public class PandaVatGenerator : EditorWindow
{
	...
	private GameObject _targetRendererObj;
	...
	[MenuItem("ぱんだスクリプト/Test")]
	static void ShowWindow()
	{
		var window = GetWindow<PandaVatGenerator>("TestEditor");
	...
	private void OnGUI()
	{
		...
		_targetRendererObj = (GameObject)EditorGUILayout.ObjectField("Target Renderer Object", _targetRendererObj, typeof(GameObject), true);
		...
		if (GUILayout.Button("Generate")) {
			if (!_rootAnim || !_animClip  || !_targetShader || !_targetRendererObj) {
			...
			if (!_targetRendererObj.transform.IsChildOf(_rootObj.transform)) {
				Debug.LogError("Error. Target Renderer is not child of Abimator.");
				return;
			}

			var isSkinnedMeshRenderer = true;
			_targetSkinnedMeshRenderer = _targetRendererObj.GetComponent<SkinnedMeshRenderer>();
			if (_targetSkinnedMeshRenderer) {
				_baseMesh = _targetSkinnedMeshRenderer.sharedMesh;
			}
			else {
				isSkinnedMeshRenderer = false;
				_targetMeshRenderer = _targetRendererObj.GetComponent<MeshRenderer>();
				...
				_baseMesh = _targetRendererObj.GetComponent<MeshFilter>()?.sharedMesh;
			}
	...
	{
		var rendererName = _targetRendererObj.name;
		...
		var renderT = _targetRendererObj.transform;
